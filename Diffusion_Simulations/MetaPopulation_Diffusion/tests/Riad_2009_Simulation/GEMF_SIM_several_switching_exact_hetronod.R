GEMF_SIM_several_switching_exact_hetronod<-function(Para,IN1,IN2,Nei1,Nei2,x0,maxNumevent,Runtime,N,numrun){
...
for(n in 1:N){
  ...
  f1<-function(trin){
   ...
   actvecvar<<-actvec;
    ...
    while((s<maxNumevent)&&(R>=1e-6)&&(Tf<Runtime)){
      ...
      if (is==1){pf<<-pf+c(0,0,beta1*h1var[ns],0)}
      if(is==2){pf<<-pf+c(0,0,0,beta1*h1var[ns]+beta2*h2var[ns])}
      ...
      if (is==2 && js==1){#s2 to s1
        ...
        R<-max(R-Rnvar[ns],0);
        Rnvar[ns]<<-di[js,ns]+beta1*h1var[ns];
        R<-R+Rnvar[ns];
        h2var[ns]<<-0;
    ...
    #-----------------------------------------    
        if (is==1 && js==2){#s1 to s2
          ...
          R<-max(R-Rnvar[ns],0);
          Rnvar[ns]<<-di[js,ns]+beta1*h1var[ns]+beta2*h2var[ns];
          R<-R+Rnvar[ns];
 ...
 #-----------------------------------------    
      ...
      if (is==4 && js==2){#I2 to s2
        ...
        R<-max(R-Rnvar[ns],0);
        Rnvar[ns]<<-di[js,ns]+h2var[ns]*beta2+h1var[ns]*beta1;
        R<-R+Rnvar[ns];
