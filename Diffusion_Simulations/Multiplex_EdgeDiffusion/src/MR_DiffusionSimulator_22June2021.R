...
# Installing Necessary Packages
  list.of.packages <- c('network','sna', 'cowplot', 'ggplot2', 'animation', 'RColorBrewer', 'scales')
...
#################################
  ...
  # GENERATING SUMMARY NETWORK (Used for both Animations and Visualizations)
    ...
    # Mapping Sequential IDs
      ...
      model$sna_network <- network::add.edges(sna_network, el[[1]],el[[2]])
      rm(source, target)
    
    # Plotting Summary Network to Get Coordinates
      par(mar = c(0,0,0,0),  family='HersheySerif')
      sna_plot <- plot(model$sna_network, mode = "kamadakawai", 
      ...
      x <- sna_plot[,1]
      y <- sna_plot[,2]
      model$inf_nodes <- cbind(model$inf_nodes, x, y)
      rm(sna_plot, x, y)
      
    # Outputting Summary Network's SNA File and plot to the global environment
      if (R_net == 1) {
        sna_infection_network <-  assign(x = "sna_infection_network", value = model$sna_network,.GlobalEnv) 
        
        out_degree <- sna::degree(sna_network, cmode="outdegree")
        model$out_degree <- out_degree + 1
        
        model$sna_infection_plot <- function() {
          par(mar = c(0,0,0,0),  family='HersheySerif')
          plot(model$sna_network, mode = "kamadakawai", 
              edge.col='grey', arrowhead.cex=0.25, edge.lwd = 0.5, 
              vertex.col=c('brown'), vertex.cex=model$out_degree * 0.15, 
              object.scale=0.02, vertices.last=TRUE, displayisolates = TRUE)
          corners = par("usr") 
          x_dist <- abs(corners[[2]] - corners[[1]])
          y_dist <- abs(corners[[4]] - corners[[3]])
          x_increment <- x_dist*0.15
          y_increment <- y_dist*0.9
          text(x=corners[1]+ x_increment, y = corners[[3]] + y_increment,'Infection Network', family='serif', font=2, cex=1.5)
        }
        
        g <- cowplot::as_grob(model$sna_infection_plot)
        p_1 <- cowplot::ggdraw(g)
        
        sna_infection_plot <-  assign(x = "sna_infection_plot", value = p_1,.GlobalEnv)
      }else{
        model$sna_network <- model$sna_network
      }
  ...
  # INFECTION NETWORKS OVER TIME
    ...
    # Adding xy coordinates to node attributes file
      for (j in seq_along(inf_attributes)) {
        t_nodes <- as.numeric(inf_attributes[[j]]$node)
        t_nodes <- model$inf_nodes[model$inf_nodes$node %in% t_nodes, ][c(2:4)]
        t_nodes$node <- as.character(t_nodes$node)
        inf_attributes[[j]] <- merge(inf_attributes[[j]], t_nodes, by.x = "node", all.x = TRUE, all.y = FALSE)
        rm(t_nodes)
      }
    ...
    # Generating sna network objects if R_net = 1
      if (R_net == 1) {
        ...
        # Generate Network Plots for each Time Period When New Infections Occurred
          model$inf_networks <- inf_networks
          model$sna_networks <- sna_networks
          model$inf_attributes <- inf_attributes
          model$degree_list <- degree_list
          model$InfNetwork_Time_Plots <- vector('list', length(model$sna_networks))
          names(model$InfNetwork_Time_Plots) <- names(model$sna_networks)
          for (j in seq_along(model$InfNetwork_Time_Plots)) {
        
            model$time_slice_plot <- function() {
                par(mar = c(0,0,0,0),  family='HersheySerif')
                plot(model$sna_networks[[j]], coord=model$inf_attributes[[j]][c(6:7)], 
                     edge.col='grey', arrowhead.cex=0.25, edge.lwd = 0.25, 
                     vertex.col=inf_attributes[[j]]$color, vertex.cex=(model$degree_list[[j]]*0.15), #label = network.vertex.names(sna_networks[[j]]),#
                     object.scale=0.02, vertices.last=TRUE, displayisolates = TRUE)
                corners = par("usr") 
                x_dist <- abs(corners[[2]] - corners[[1]])
                y_dist <- abs(corners[[4]] - corners[[3]])
                x_increment <- x_dist*0.2
                y_increment <- y_dist*0.9
                text(x=(corners[1]+  x_increment), y = (corners[[3]] + y_increment), paste(names(model$inf_networks)[[j]], 'Infection Network'), 
                     family='serif', font=2, cex=1.5)
            }
        
            g <- cowplot::as_grob(model$time_slice_plot)
            p_1 <- cowplot::ggdraw(g)
          
            model$InfNetwork_Time_Plots[[j]] <- p_1
            rm(g, p_1)
            Sys.sleep(1)
          }
        
        # Outputting sna Temporal Networks
          sna_timeslice_networks <-  assign(x = "sna_timeslice_networks", value = model$sna_networks,.GlobalEnv) 
        
        # Create HTML Movie showing changes in the infection network over time
          animation::saveHTML({
            for(j in seq_along(model$InfNetwork_Time_Plots)) {                      
              plot(model$InfNetwork_Time_Plots[[j]], add=T)
            }
          }, htmlfile = "InfectonNetwork_OverTime.html", autoplay=FALSE, verbose=FALSE,
          ani.width = 877, ani.height=676, nmax=length(model$InfNetwork_Time_Plots))
  ...
  # CELLULAR AUTOMATA: LATTICE GRAPH
    if (cellular_net == 1 & dim(coordinates)[[2]] >= 3) {
      # Isolating Connected Components from Summary Network
        cd <- sna::component.dist(model$sna_network, connected="weak")
        model$sna_network %v% "component" <- cd$membership   
        model$inf_nodes$component <- cd$membership    
        rm(cd)
        
      # Calculate Out-Degree 
        out_degree <- sna::degree(model$sna_network, cmode="outdegree")
        model$inf_nodes$out_degree <- out_degree
        rm(out_degree)
        
      # Creating Components List for the Purposes of Plotting the Sub-Networks
        components_list <- vector('list', length(unique(model$inf_nodes$component)))
        names(components_list) <- unique(model$inf_nodes$component)
        for (j in seq_along(components_list)) {
          components_list[[j]] <- model$inf_nodes[model$inf_nodes$component == j, ]
        }
      ...
      # Plot Nodes from the Coordinates List
        ...
        model$cellular_data <- cellular_data
        model$edges <- edges
       
        model$lattice_graph <- function() {
          # Base Layout
            layout(rbind(1,2), heights=c(9.9,0.1))
            par(mar = c(1,0,1,1),  family='HersheySerif')
            plot(type='n', x=dim(model$cellular_data)[[1]], y=dim(model$cellular_data)[[2]], xlim=c(1, dim(model$cellular_data)[[1]]), 
                  ylim=c(1, dim(model$cellular_data)[[2]]), axes=FALSE, xlab=' ', ylab= '', bty='n')
        
          # Adding Points
            for(j in seq_along(model$coordinates$node)) {
              points(x=model$coordinates[j,2], y=model$coordinates[j,3], pch=coordinates[j,5], col=model$coordinates[j,4], cex=1)
            }
        
          # Adding Arrows
            model$edges <- model$edges[model$edges$value != 0, ]
            for ( j in seq_along(model$edges$n1)) {
              edge <- model$edges[j,c(2,3)]
              ...
              arrows(edge_list[[1]]$x, edge_list[[1]]$y, edge_list[[2]]$x, edge_list[[2]]$y, 
                     length = 0.09, col='grey42', code=2, lwd=2)
          ...
          # Adding Legend
            ...
            plot.new()
            text(x=0.35, y = 3, "Seeds")
            points(x=0.4, y=3, pch=0)
            text(x=0.65, y = 3, "Nodes")
            points(x=0.7, y=3, pch=1)
        } 
        
        g <- cowplot::as_grob(model$lattice_graph)
        p_1 <- cowplot::ggdraw(g)
  ...
  # INFECTION GROWTH CURVE
    ...
    # Plotting
      model$infection_counts <- infection_counts
      
      model$growth_curve <- function() {
      ...
      g <- cowplot::as_grob(model$growth_curve)
      p_1 <- cowplot::ggdraw(g)
