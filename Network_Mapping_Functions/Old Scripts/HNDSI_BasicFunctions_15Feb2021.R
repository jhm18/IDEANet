...
#   Multiplex network require an edgelist specification with a four column, type, indicating the different tie types.
      ...
      # Create Graph Objects
        if(package == 'igraph'){
            ...
            trans_rate <- function(g) {
              # Isolating One-Step Paths
                ...
                for(i in seq_along(one_step_paths)){
                  one_step_paths[[i]] <- igraph::neighborhood(g, order=1, mindist = 1, igraph::V(g)[[i]], mode='all')[[1]]
                }
              ...
              # Isolating Two-Step Paths
                ...
                for(i in seq_along(two_step_paths)){
                  two_step_paths[[i]] <- igraph::neighborhood(g, order= 2, mindist = 1, igraph::V(g)[[i]], mode='all')[[1]]
                }
              ...
              # Calculating the Proportion of Two-Step Path that Are Also One-Step Paths
                ...
                for(i in seq_along(proportion_two_step)) {
                  # Identifying Nodes that Occur in Both Two and One-Step Paths
                    shared_paths <- sort(intersect(as.integer(names(one_step_paths[[i]])), as.numeric(names(two_step_paths[[i]]))))
              ...
              # Transitivity Rate
                proportion_two_step[is.na(proportion_two_step)] <- 0
          ...
          # Calculating System-Level Measures
            ...
            transitivity_rate <- trans_rate(g)
...
# Multiplex Network Example
  ...
  multiplex_edge_corr <- function(edgelist) {
    if(){
